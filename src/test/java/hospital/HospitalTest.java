...
public class HospitalTest {
    ...
    Doctor doctor = new Doctor("1", "Joe Buck", "brain");
    Nurse nurse = new Nurse("2", "Nurse Betty", 5);
    Janitor janitor = new Janitor("3", "Joe Dirt", "Emergency", true);

    @Test
    public void shouldBeAbleToHireADoctor(){
        underTest.hire(doctor);
        HospitalEmployee foundDoctor = underTest.findEmployee("1");
        assertEquals("Joe Buck", foundDoctor.getName());
    }
    ...
    public void shouldBeAbleToHireDoctorNurseAndJanitor(){
        ...
        assertEquals("Joe Buck", foundDoctor.getName());
        assertEquals("Nurse Betty", foundNurse.getName());
        assertEquals("Joe Dirt", foundJanitor.getName());
    }
